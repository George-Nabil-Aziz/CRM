# Implementation Plan: Contact via SMS
...
- **Message**: id, ticketId, channel=sms, from, body, receivedAt

## API Surface

See `contracts/api.md` — `POST /api/channels/sms/inbound`

**Request**: SMS provider webhook payload
...
2. Implement `POST /api/channels/sms/inbound` per contracts/api.md, including validation and the error responses listed in its Status Codes table.
3. Enforce authorization on the endpoint so only permitted callers can invoke it.
4. Build the UI/flow that lets a(n) Customer text support.
5. Handle SMS delivery-failure callbacks from the provider.
