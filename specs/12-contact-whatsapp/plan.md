# Implementation Plan: Contact via WhatsApp
...
- **Message**: id, ticketId, channel=whatsapp, from, body, receivedAt

## API Surface

See `contracts/api.md` — `POST /api/channels/whatsapp/inbound`

**Request**: WhatsApp Business API webhook payload
...
2. Implement `POST /api/channels/whatsapp/inbound` per contracts/api.md, including validation and the error responses listed in its Status Codes table.
3. Enforce authorization on the endpoint so only permitted callers can invoke it.
4. Build the UI/flow that lets a(n) Customer message support on WhatsApp.
5. Support WhatsApp media attachments (images/documents).
