# Feature Specification: Track requests
...
As a(n) **Customer**, I want to **track the status of my requests**, so that **I know progress without contacting support**.

## Description

This story covers the `GET /api/portal/tickets/{id}` capability in the Customer Portal area. Same ticket-read endpoint agents use, restricted to tickets owned by the logged-in customer.

## Acceptance Criteria

1. **Happy path** — **Given** an open ticket, **When** the customer views the portal, **Then** its current status is shown
...
4. **Authorization** — Given the caller does not have permission to view this data, **When** they call `GET /api/portal/tickets/{id}`, **Then** the system returns 401/403 and no data is returned.

## Functional Requirements

- **FR-36-1**: System MUST allow a(n) Customer to track the status of my requests via `GET /api/portal/tickets/{id}`.
- **FR-36-2**: System MUST validate every field listed under Requirements > Data Fields below and reject invalid requests with a 400 error.
- **FR-36-3**: System MUST ensure this action is only performed by an authorized caller (matching role/permission) or an authenticated system process.
- **FR-36-4**: System MUST push a status-change notification to the customer automatically.
- **FR-36-5**: Changes made by this story MUST be immediately visible through the corresponding read endpoint(s) for the Customer Portal area.
...
| `status` | string (enum) | No | Read-only in this response; one of: `open`, `pending`, `resolved`, `closed`. |
