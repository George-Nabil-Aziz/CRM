# Feature Specification: Submit via web form
...
As a(n) **Customer**, I want to **submit a web form**, so that **I can report an issue from the website without logging in**.

## Description

This story covers the `POST /api/channels/webform` capability in the Communication Channels area. Validate and rate-limit the public form endpoint since it is unauthenticated.

## Acceptance Criteria

1. **Happy path** — **Given** a customer fills out a web form, **When** they submit it, **Then** a ticket is created from the form data
...
- **FR-15-1**: System MUST allow a(n) Customer to submit a web form via `POST /api/channels/webform`.
- **FR-15-2**: System MUST validate every field listed under Requirements > Data Fields below and reject invalid requests with a 400 error.
- **FR-15-3**: System MUST ensure this action is only performed by an authorized caller (matching role/permission) or an authenticated system process.
- **FR-15-4**: System MUST add a spam/CAPTCHA check before ticket creation.
- **FR-15-5**: Changes made by this story MUST be immediately visible through the corresponding read endpoint(s) for the Communication Channels area.
...
| `channel` | string (fixed) | No | Always set to `webform` by this handler; not caller-supplied. |
